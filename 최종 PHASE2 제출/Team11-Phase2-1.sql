...
CREATE TABLE GAME_SESSION (
    ...
    PERSONAL_DEBT NUMBER(9) DEFAULT 500000, -- 남은(갚아야 하는) 개인 빚
        CONSTRAINT CK_PERSONAL_DEBT_RANGE CHECK(PERSONAL_DEBT BETWEEN 0 AND 500000),
    ...
    CONSTRAINT CK_UNLOCKED_SHOWCASE_COUNT_RANGE CHECK (UNLOCKED_SHOWCASE_COUNT IN (2, 4, 8)),
    NICKNAME VARCHAR2(10 CHAR), -- 10글자 내 이름(한글 고려)
    SHOP_NAME VARCHAR2(10 CHAR), -- 10글자 내 상점 이름(한글 고려)
    -- 클리어 나온 뒤 기록
    GAME_END_DAY_COUNT NUMBER(5) DEFAULT NULL, -- 게임 종료 시 게임 내 일수.
        -- 클리어 시 양수, 게임 오버 시 음수, NULL은 게임 중
    GAME_END_DATE DATE DEFAULT NULL -- 클리어 당시 현실 날짜
);
...
CREATE TABLE EXISTING_ITEM(
    ...
    GAME_SESSION_KEY NUMBER(9) NOT NULL REFERENCES GAME_SESSION(GAME_SESSION_KEY), --FK 게임세션 키. 해당 게임세션에서 할당된 아이템
    ...
    SUSPICIOUS_FLAW_AURA NUMBER(5,4) DEFAULT 0 NOT NULL, -- 아이템에서 풍기는 수상한 흠 기운 정도 0.0000 ~ 1.0000
        CONSTRAINT CK_SUSPICIOUS_FLAW_AURA_RANGE CHECK(SUSPICIOUS_FLAW_AURA BETWEEN 0.0 AND 1.0),
...
CREATE TABLE NEWS_CATALOG(
    ...
    NEWS_DESCRIPTION VARCHAR2(50 CHAR) NOT NULL UNIQUE,
...
CREATE TABLE EXISTING_NEWS(
    GAME_SESSION_KEY NUMBER(9) NOT NULL REFERENCES GAME_SESSION(GAME_SESSION_KEY) ON DELETE CASCADE,
    ...
    -- 게임 세션 당 뉴스 카탈로그를 구분(Weak Entity)
);
