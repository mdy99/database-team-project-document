...
WHERE P.PLAYER_ID = G.PLAYER_ID AND G.GAME_SESSION_KEY = D.GAME_SESSION_KEY;
-- 게임 (진행 중, 완료 포함)의 일자별 총이익 (판매) 합계: Player 이름과 함께 (Type 3)
SELECT P.PLAYER_ID, SUM(D.PURCHASE_PRICE) FROM (
    ((PLAYER P JOIN GAME_SESSION G ON P.PLAYER_ID = G.PLAYER_ID)
...
-- 각 Item 종류별 게임 세션에 등장 횟수 (Type 7)
SELECT ITEM_CATALOG_NAME, COUNT(*) FROM (
    SELECT ITEM_CATALOG_KEY, ITEM_CATALOG_NAME FROM ITEM_CATALOG IC, EXISTING_ITEM I
    WHERE IC.ITEM_CATALOG_KEY = I.ITEM_CATALOG_KEY
) ALL_ITEMS
GROUP BY ITEM_CATALOG_NAME
ORDER BY ITEM_CATALOG_NAME;
-- 플레이어별 각 게임 세션별 아이템 개수 (순이익 높은 순서대로) (Type 9)
SELECT P.PLAYER_ID, S.GAME_SESSION_KEY, COUNT(*)
FROM ((PLAYER P JOIN GAME_SESSION G ON P.PLAYER_ID = G.PLAYER_ID)
...
(
    ...
    WHERE
        ...
        AND C.ITEM_CATALOG_KEY = IC.ITEM_CATALOG_KEY
);
